...
|---|---|---|---|---|---|
| `sync_payment_files(cursor, cfg, token, interface_id, from_date=None, to_date=None)` | live `cursor`; `cfg: AppConfig`; `token: str`; `interface_id: int`; optional `from_date`/`to_date: date` | `list[int]` — every `ap_payment_file_details.id` inserted this run | `_to_api_datetime`, `_fetch_all_pages`, `get_payment_batches`, `insert_payment_file`, `get_invoice_list`, `insert_batch_invoice_detail`, `insert_batch_invoice_allocation_value`, `insert_batch_invoice_custom`, `insert_ap_invoice` | — | Date window: `[today - GetPaymentBatches.LookbackDays, today]` (UTC) by default, or `[from_date, to_date]` when both are given (threaded from `process_invoices.py`'s optional CLI date argument(s)). Each bound is run through `_to_api_datetime()` before being sent as `fromDate`/`toDate` — the API rejects a bare `YYYY-MM-DD` and requires ISO 8601 UTC (`yyyy-MM-ddTHH:mm:ssZ`). No dedup: every batch the API returns gets a brand-new `ap_payment_file_details` row every run, even if its `paymentFileId` was already seen on a prior run — deliberate, since a batch's invoice list can grow or shrink on the vendor's side and the whole chain is cheaper to reprocess from a fresh id than to diff. Reprocessing is naturally bounded by the date window itself: once a batch ages out of `[today - LookbackDays, today]`, the API stops returning it |
| `_to_api_datetime(d: date, end_of_day=False)` | `d: date`; `end_of_day: bool` | `str` — `yyyy-MM-ddTHH:mm:ssZ` | — | `datetime.combine/strftime` | `00:00:00Z` by default, `23:59:59Z` when `end_of_day=True` (so a `to_date` bound covers that whole day) |
...
called with `expand=true` — `GetInvoiceList.Expand` in `appsettings.yml`, so the
response includes related invoice details)
...
|---|---|---|---|---|---|
| `main(interface_id=None, from_date=None, to_date=None)` | `interface_id: int \| None`; `from_date`/`to_date: date \| None`; if `interface_id` is `None`, all three are parsed from `--interface-id` + an optional trailing `dates` positional (0, 1, or 2 `YYYY-MM-DD` values) | `None` (raises on setup failure) | `load_config`, `setup_logging`, `get_connection`, `load_interface_configuration`, `authenticate`, `sync_payment_files`, `get_open_payment_files`, `process_payment_file` | `argparse`, `mysql.connector` (cursor), `traceback` | Opens/closes DB conn + cursor; configures root logger; drives per-file loop. `from_date`/`to_date` (still calendar dates here) are passed through to `sync_payment_files`, which converts them via `_to_api_datetime()` before calling the API |
